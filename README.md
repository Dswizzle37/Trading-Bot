...
- **Resend** — sign up at [resend.com](https://resend.com) and create an API key. Used for email notifications over HTTPS. (Gmail SMTP does *not* work from cloud routines — those sandboxes only tunnel HTTPS/443, so port 465 always times out.)
- **GitHub token** — generate a fine-grained PAT at [github.com/settings/personal-access-tokens/new](https://github.com/settings/personal-access-tokens/new), scoped to only `Dswizzle37/Trading-Bot`, with **Contents: Read and write**. The routines push with this instead of Claude's GitHub App, which currently 403s on all writes ([known issue](https://github.com/anthropics/claude-code/issues/80874)).

### 2. Local smoke test
```bash
cp env.template .env
```
Open `.env` and paste in your real Alpaca paper / Perplexity / Resend / GitHub token values yourself (don't paste them into chat — I never handle credentials). Then in Claude Code, run:
```
/portfolio
```
